...
// Will not order items/keep order.
export const itemMultiSelect = (itemIds, name) => `
<select class="item-select2" multiple name="${name}">
    ${itemIds.reduce((html, itemId) => {
            let bankItem = getBankItem(itemId)
            let qty = bankItem ? bankItem.qty : 0
            html += `<option value="${itemId}" data-img="${items[itemId].media}">${items[itemId].name} (${qty})</option>`
            return html
        }, ''
    )}
</select>
...
export const itemSelect = (itemIds, name) => select2Single(itemIds.map(itemId => {
    let bankItem = getBankItem(itemId)
    if (!itemId) {
        return {
            img: null,
...
export const select2Single = (options, name) => `
<select class="item-select2" name="${name}">
...
<div class="media d-flex align-items-center push">
    ...
    <div class="media-body">
        ...
        <div class="font-size-sm text-success d-none" id="harvest-ready-0">${subtext}</div>
    </div>
</div>
`, classList, skill)}`
